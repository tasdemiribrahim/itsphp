	...
	init : function(){
		...
		$("#loginForm").validate({
			...
			submitHandler: function(form) {
				...
				working = true;
				$.itPOST('login',$("#loginForm").serialize(),function(r){
					working = false;
					if(r.error)
		...
		$("#submitForm").validate({
				debug: false,
				errorLabelContainer: $("#uyari"),
				rules: {	chatText: "required" },
				messages: {chatText: "Bir mesaj girin!<br />"},
				submitHandler: function(form) {
					...
					$.itPOST('submitChat',$("#submitForm").serialize(),function(r){
						working = false;
		...
		$('a.logoutButton').live('click',function(){
			$('#chatTopBar > span').fadeOut(function(){ $(this).remove();});
			$('#submitForm').fadeOut(function(){$('#loginForm').fadeIn();});
			$.itPOST('logout');
			//socket.close();
			//window.log("logout");
			return false;
...
//}
	...
	getChatsAdd : function(r,callback){
		for(var i=0;i<r.chats.length;i++)
			...
			if(!chat.data.lastID)
				$('#chatLineHolder').html('<p class="noChats">Mesaj yok!</p>');
	...
	displayError : function(msg){
		var elem = $('<div>',{id : 'chatErrorMessage',html: msg});
...
$.itPOST = function(action,data,callback){
	$.post('/main/iletisim/ajax?chat='+action,data,callback,'json');
}

$.itGET = function(action,data,callback){
	$.get('/main/iletisim/ajax?chat='+action,data,callback,'json');
}
